...
const workItemsPath = "packages/work-items/src/pr30-authority-regressions.test.ts";
let source = readFileSync(workItemsPath, "utf8");
source = source.replace('import { createPolicyEngine, createWorkItemTools } from "@agent-control-stack/policy-gate";\n', "");
const cancelStart = '  it("cancelling a running authoritative work item revokes its attempt lease and terminalizes the attempt", () => {';
const start = source.indexOf(cancelStart);
if (start < 0) throw new Error("cancel regression block not found");
const endMarker = "\n  });\n});\n";
const end = source.indexOf(endMarker, start);
if (end < 0) throw new Error("cancel regression block end not found");
source = `${source.slice(0, start)}});\n`;
writeFileSync(workItemsPath, source);

writeFileSync(
  "packages/policy-gate/src/pr30-cancel-regression.test.ts",
