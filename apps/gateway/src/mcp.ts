...
import { type createGatewayWorkItemTools, workItemToolNames } from "./tools/work-items.js";

const MCP_PROTOCOL_VERSION = "2024-11-05";

type GatewayWorkItemTools = ReturnType<typeof createGatewayWorkItemTools>;
...
function toolDescription(name: GatewayToolName): string {
  switch (name) {
    ...
    case "submit_work_result":
      return "Submit a worker result after policy and approval checks.";
  }
}
...
function errorStatus(error: unknown): number {
  ...
  if (error instanceof ControlStackError) {
    return error.code === "work_item_not_found" ? 404 : 409;
  }
  return 500;
}
